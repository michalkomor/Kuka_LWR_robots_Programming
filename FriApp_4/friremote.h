...
public:
    ...
    */
    ...
    {
        return msr.data.cmdCartPos;
    }

    /** commanded Cartesian frame (in m, due to FRI) */
    float * getMsrCmdCartPosFriOffset()
    {
        return msr.data.cmdCartPosFriOffset;
    }
    ...
    float * getMsrCmdJntPosition()
    {
        return msr.data.cmdJntPos;
    }
    ...
    *  Complete desired position inside LBR Kernel is the sum of cmdJntPos and cmdJntPosFriOffset */
    float * getMsrCmdJntPositionOffset()
    {
        return msr.data.cmdJntPosFriOffset;
    }

    void getCurrentCmdJntPosition( float jntVec[LBR_MNJ] )
    {
        for ( int i = 0; i < LBR_MNJ; i++) jntVec[i]= msr.data.cmdJntPos[i]+msr.data.cmdJntPosFriOffset[i];
    }
    /** Current measured jnt position of the robot */
    float * getMsrMsrJntPosition()
    {
        return msr.data.msrJntPos;
    }
    float * getMsrEstExtJntTrq()
    {
        return msr.data.estExtJntTrq;
    }
    float * getMsrJntTrq()
    {
        return msr.data.msrJntTrq;
    }
    float * getMsrEstExtTcpFT()
    {
        return msr.data.estExtTcpFT;
    }
    /* @} */
